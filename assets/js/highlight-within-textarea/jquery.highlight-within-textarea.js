...
(function($) {
	...
	HighlightWithinTextarea.prototype = {
		...
		getRegExpRanges: function(input, regex) {
			let ranges = [];
			let match;
			if(String(regex) == String(/(?:)/g))
				regex = /a^/g; //match noting
				...
				if (match.length == 1){
					...
					for (let i=1; i<match.length ; i++){
						if(match[i] != undefined){
							let r = []
							r.push([match.index + match[0].indexOf(match[i]), match.index + match[0].indexOf(match[i]) + match[i].length])
							ranges.push(r)
						}
					}
				}
			...
			if(match = regex.exec(input), match !== null){
				if(match.length >1){
					return this.getArrayRanges(input, ranges)
				}
				return ranges;
			}
			return ranges;
